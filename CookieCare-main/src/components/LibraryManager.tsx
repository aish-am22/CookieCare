    ...
    <div className="flex-1 flex flex-col h-full bg-[#fafafa] relative overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto px-8 py-8 z-10 relative">
        ...
        <div className="bg-white border border-gray-200/80 rounded-xl p-6 mb-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">{activeTabInfo.label}</h2>
        ...
        {activeTab === "saved-drafts" ? (
          <div className="bg-white border border-gray-250/70 rounded-b-xl overflow-hidden shadow-xs flex-1 flex flex-col">
            {filteredDrafts.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[#fafafa]/50 min-h-[300px]">
                ...
                <h4 className="text-xs font-mono font-bold text-gray-850 uppercase">No saved drafts found</h4>
              ...
              <>
                  ...
                  <table className="w-full text-left border-collapse min-w-[600px]">
                    <thead>
                      <tr className="border-b border-gray-200 bg-[#fbfbfb] text-xs font-mono font-bold text-gray-500 select-none">
                        <th className="px-6 py-3 w-2/5">Draft Name</th>
                        <th className="px-5 py-3 text-center w-40">Last Updated</th>
                        <th className="px-5 py-3 text-center w-40">Created Date</th>
                        <th className="px-6 py-3 text-center">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100/80 text-xs text-gray-700 font-sans">
                      {filteredDrafts.map((draft) => (
                        <tr key={draft.id} className="hover:bg-gray-50/70 transition group">
                          <td className="px-6 py-4 font-semibold text-gray-900 group-hover:text-black">
                            <div className="flex items-center space-x-3">
                              <FilePen className="w-4 h-4 text-indigo-400 shrink-0" />
                              <span className="truncate max-w-[280px] select-all">{draft.title}</span>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4 text-center">
                            <div className="flex items-center justify-center space-x-1.5 opacity-50 group-hover:opacity-100 transition">
                              {/* Open in Editor */}
                              {onOpenInDraftEditor && (
                                <button
                                  title="Open in Draft Editor"
                                  onClick={() => onOpenInDraftEditor(draft as LegalDocument)}
                                  className="p-1 px-2 border border-gray-200 bg-white hover:border-black hover:bg-gray-50 rounded text-gray-600 hover:text-black shrink-0 cursor-pointer flex items-center gap-1 text-[10px] font-mono font-bold"
                                >
                              ...
                              <button
                                title="Copy ID"
                                onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(draft.id); setCopiedId(draft.id); setTimeout(() => setCopiedId(null), 1500); }}
                              ...
                              >
                                {copiedId === draft.id ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                              </button>
                              {/* Delete */}
                              <button
                                title="Delete Draft"
                ...
                <div className="border-t border-gray-200 bg-[#fdfdfd] px-6 py-3.5 text-xs font-mono text-gray-500 select-none">
                  Showing all {filteredDrafts.length} {filteredDrafts.length === 1 ? "entry" : "entries"}
                </div>
              </>
            )}
          </div>
          ...
          <div className="bg-white border border-gray-250/70 rounded-b-xl overflow-hidden shadow-xs flex-1 flex flex-col">
            {sortedItems.length === 0 ? (
              ...
              <div className="flex-1 overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead>
                    <tr className="border-b border-gray-200 bg-[#fbfbfb] text-xs font-mono font-bold text-gray-500 select-none">
                      <th onClick={() => toggleSort("name")}        className="px-6 py-3 cursor-pointer hover:bg-gray-150 transition truncate w-1/4"><div className="flex items-center gap-1"><span>Name</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      <th onClick={() => toggleSort("description")} className="px-6 py-3 cursor-pointer hover:bg-gray-150 transition w-1/3"><div className="flex items-center gap-1"><span>Description</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      <th onClick={() => toggleSort("tags")}        className="px-5 py-3 cursor-pointer hover:bg-gray-150 transition w-12"><div className="flex items-center gap-1"><span>Tags</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      <th onClick={() => toggleSort("itemsCount")}  className="px-5 py-3 cursor-pointer hover:bg-gray-150 transition text-center w-24"><div className="flex items-center gap-1 justify-center"><span>Items</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      <th onClick={() => toggleSort("dateModified")}className="px-5 py-3 cursor-pointer hover:bg-gray-150 transition text-center w-32"><div className="flex items-center gap-1 justify-center"><span>Date modified</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      <th onClick={() => toggleSort("createdBy")}   className="px-5 py-3 cursor-pointer hover:bg-gray-150 transition w-32"><div className="flex items-center gap-1"><span>Created by</span><Sliders className="w-2.5 h-2.5 rotate-90" /></div></th>
                      ...
                      >
                        <td className="px-6 py-4 font-semibold text-gray-900 group-hover:text-black">
                          <div className="flex items-center space-x-3.5">
                            {item.type === "files"     ? <Folder    className="w-4.5 h-4.5 text-gray-400 group-hover:text-amber-500 transition shrink-0" />
                            : item.type === "prompts"  ? <Sparkles  className="w-4.5 h-4.5 text-emerald-500 shrink-0" />
                            : item.type === "questions"? <BookOpen  className="w-4.5 h-4.5 text-amber-500 shrink-0" />
                            : item.type === "websites" ? <Globe     className="w-4.5 h-4.5 text-blue-500 shrink-0" />
                            :                           <FileText   className="w-4.5 h-4.5 text-indigo-400 shrink-0" />}
                            <span className="truncate max-w-[220px] select-all">{item.name}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-gray-500 leading-relaxed font-normal max-w-[320px] truncate select-all">{item.description}</td>
                        <td className="px-5 py-4">
                          <span className={`inline-block px-2.5 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-tight ${item.tags === "-" ? "text-gray-400 bg-gray-50" : "text-indigo-800 bg-indigo-50/75 border border-indigo-100"}`}>
                            {item.tags}
                          </span>
                        </td>
                        <td className="px-5 py-4 font-mono text-center text-gray-600 select-all">{item.type === "files" ? item.fileList?.length : item.itemsCount}</td>
                        <td className="px-5 py-4 font-mono text-center text-gray-400 select-all">{item.dateModified}</td>
                        <td className="px-5 py-4 font-medium text-gray-800 select-all">{item.createdBy}</td>
                        <td className="px-6 py-4 text-center">
                          <div className="flex items-center justify-center space-x-1.5 opacity-40 group-hover:opacity-100 transition">
                            <button title="Copy Key Reference ID" onClick={(e) => handleCopyId(item.id, e)} className="p-1 px-1.5 border border-gray-150 bg-white hover:border-black rounded text-gray-500 hover:text-black shrink-0 cursor-pointer">
                              {copiedId === item.id ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                            </button>
                            <button title="Delete Item" onClick={(e) => handleDeleteItem(item.id, e)} className="p-1.5 bg-white text-gray-400 hover:text-rose-600 border border-gray-150 rounded shrink-0 cursor-pointer">
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </td>
            ...
            <div className="border-t border-gray-200 bg-[#fdfdfd] p-4.5 px-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs font-mono text-gray-500 select-none">
              <div>Showing all {sortedItems.length} entries</div>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-1.5">
                  <span>Records per page:</span>
                  <select value={recordsPerPage} onChange={(e) => { setRecordsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-white border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none focus:border-black cursor-pointer font-sans">
                    <option value={5}>5</option><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option>
                  </select>
                </div>
                <div className="flex items-center space-x-1.5">
                  <button disabled={currentPage === 1}          onClick={() => setCurrentPage(1)}                                      className="p-1 cursor-pointer border border-gray-200 bg-white rounded hover:border-black disabled:opacity-30 disabled:pointer-events-none transition"><ChevronsLeft  className="w-3.5 h-3.5 text-gray-600" /></button>
                  <button disabled={currentPage === 1}          onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}             className="p-1 cursor-pointer border border-gray-200 bg-white rounded hover:border-black disabled:opacity-30 disabled:pointer-events-none transition"><ChevronLeft   className="w-3.5 h-3.5 text-gray-600" /></button>
                  <span className="text-gray-800 font-bold px-1.5">{currentPage} <span className="font-normal text-gray-400">of</span> {totalPages}</span>
                  <button disabled={currentPage === totalPages} onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))}    className="p-1 cursor-pointer border border-gray-200 bg-white rounded hover:border-black disabled:opacity-30 disabled:pointer-events-none transition"><ChevronRight  className="w-3.5 h-3.5 text-gray-600" /></button>
                  <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(totalPages)}                             className="p-1 cursor-pointer border border-gray-200 bg-white rounded hover:border-black disabled:opacity-30 disabled:pointer-events-none transition"><ChevronsRight className="w-3.5 h-3.5 text-gray-600" /></button>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* MODAL 1: CREATE PERSONALIZATION */}
      {isCreateOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-lg bg-white border border-gray-200/90 shadow-2xl p-6 rounded-xl relative select-none">
            <button onClick={() => setIsCreateOpen(false)} className="absolute right-4 top-4 p-1.5 text-gray-400 hover:text-black cursor-pointer transition border border-gray-100 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
            <form onSubmit={handleCreateNewItem} className="space-y-4">
              <div className="pb-2 border-b border-gray-100">
                ...
                <h3 className="font-display font-extrabold text-lg text-gray-900 mt-0.5">Create {activeTabInfo.buttonWord}</h3>
              </div>
              <div className="space-y-3.5">
                <div>
                  <label className="block text-xs font-mono uppercase text-gray-500 mb-1">Guideline Label Name (Required)</label>
                  <input type="text" required placeholder={activeTab === "websites" ? "e.g. EU GDPR Legal Gazette" : "e.g. Acme standard subprocessor notice guidelines"} value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full bg-[#fcfcfc] border border-gray-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-black" />
                </div>
                <div>
                  <label className="block text-xs font-mono uppercase text-gray-500 mb-1">Description summary (Optional)</label>
                  <input type="text" placeholder="Brief scope detailing when this personalization is selected" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} className="w-full bg-[#fcfcfc] border border-gray-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-black" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-mono uppercase text-gray-500 mb-1">Classification Tag (Optional)</label>
                    <input type="text" placeholder="e.g. Liability, Tax, GDPR" value={formTags} onChange={(e) => setFormTags(e.target.value)} className="w-full bg-[#fcfcfc] border border-gray-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-black" />
                  </div>
                  <div>
                    <label className="block text-xs font-mono uppercase text-gray-500 mb-1">Resource Category</label>
                    <input type="text" disabled value={activeTabInfo.label} className="w-full bg-gray-50 text-gray-400 border border-gray-150 rounded-lg p-2.5 text-xs font-semibold focus:outline-none capitalize" />
                  </div>
                </div>
                {activeTab !== "files" && activeTab !== "tags" && (
                  <div>
                    <label className="block text-xs font-mono uppercase text-gray-500 mb-1">{activeTab === "websites" ? "Target Web URL Address" : "System Directive Instructions / Boilerplate Text"}</label>
                    <textarea rows={4} required placeholder={activeTab === "websites" ? "https://..." : activeTab === "prompts" ? "Enter strict AI system prompts context here..." : activeTab === "questions" ? "Enter pre-configured context questions (one per line)..." : "Enter standard clauses wording or preambles templates here..."} value={formDetails} onChange={(e) => setFormDetails(e.target.value)} className="w-full bg-[#fcfcfc] border border-gray-200 rounded-lg p-2.5 text-xs focus:outline-none focus:border-black font-mono leading-relaxed" />
                  </div>
                )}
              </div>
              <div className="flex space-x-2.5 pt-3 border-t border-gray-100 justify-end">
                <button type="button" onClick={() => setIsCreateOpen(false)} className="px-4 py-2 border border-gray-200 text-gray-500 hover:text-black rounded-lg text-xs font-mono uppercase tracking-tight bg-white hover:bg-gray-50 transition cursor-pointer">Cancel</button>
                <button type="submit" className="px-5 py-2 bg-black hover:bg-gray-800 text-white rounded-lg text-xs font-mono uppercase tracking-tight transition shadow-md cursor-pointer">Create guidelines</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {selectedFolder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-2xl bg-white border border-gray-200/90 shadow-2xl p-6 rounded-xl relative select-none">
            <button onClick={() => setSelectedFolder(null)} className="absolute right-4 top-4 p-1.5 text-gray-400 hover:text-black cursor-pointer transition border border-gray-100 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
            <div className="mb-5 pb-3.5 border-b border-gray-100 flex items-start gap-3">
              <div className="bg-amber-50 p-2.5 text-amber-500 rounded-lg border border-amber-100"><Folder className="w-6 h-6 text-amber-500" /></div>
              ...
              <button onClick={() => { setFormFolderTarget(selectedFolder.id); setIsAddFileOpen(true); }} className="px-3.5 py-2 bg-black hover:bg-gray-800 text-white text-xs font-mono font-bold transition rounded-lg shrink-0 flex items-center gap-1 cursor-pointer">
                <Upload className="w-3.5 h-3.5 text-white" /><span>Add Files</span>
              </button>
            </div>
            <h4 className="text-[10px] font-mono font-bold text-gray-500 uppercase tracking-wider mb-2.5">Files List ({selectedFolder.fileList?.length || 0} items)</h4>
            {(!selectedFolder.fileList || selectedFolder.fileList.length === 0) ? (
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 border border-gray-100 p-2.5 rounded-xl bg-gray-50/50">
                {selectedFolder.fileList.map((file, idx) => (
                  <div key={idx} className="bg-white border border-gray-200/95 rounded-lg p-3 flex justify-between items-center hover:border-gray-450 transition text-xs font-mono">
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      ...
                      <button onClick={() => handleDeleteFileFromFolder(selectedFolder.id, file.name)} className="p-1 text-gray-450 hover:text-rose-600 transition cursor-pointer hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-md" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-end space-x-2.5 pt-4 border-t border-gray-100 mt-5">
              <button onClick={(e) => handleDeleteItem(selectedFolder.id, e)} className="px-4 py-2 hover:bg-rose-50 hover:text-rose-600 border border-transparent rounded-lg text-xs font-mono uppercase tracking-tight transition cursor-pointer text-gray-450">Delete Folder</button>
              <button onClick={() => setSelectedFolder(null)} className="px-5 py-2 border border-gray-250 bg-white hover:border-black rounded-lg text-xs text-gray-800 font-mono uppercase tracking-tight transition cursor-pointer shadow-sm">Close Dock</button>
            </div>
          </div>
        </div>
      )}
      ...
      {viewDetailItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-lg bg-white border border-gray-200/90 shadow-2xl p-6 rounded-xl relative select-none">
            <button onClick={() => setViewDetailItem(null)} className="absolute right-4 top-4 p-1.5 text-gray-400 hover:text-black cursor-pointer transition border border-gray-100 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
            <div className="mb-5 pb-3.5 border-b border-gray-100 flex items-start gap-3">
              <div className="bg-indigo-50 p-2.5 text-indigo-500 rounded-lg border border-indigo-150 shrink-0"><FileCode className="w-6 h-6 text-indigo-500" /></div>
              <div className="min-w-0">
                <span className="text-[10px] font-mono text-gray-400 uppercase tracking-widest block">PERSONALIZATION BRIEFCASE</span>
                <h3 className="font-display font-extrabold text-base text-gray-900 mt-0.5 select-all leading-tight">{viewDetailItem.name}</h3>
            ...
            <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
              <div>
                ...
                <p className="text-xs text-gray-600 leading-relaxed bg-[#fdfdfd] border border-gray-150 rounded-lg p-3 font-medium select-all">{viewDetailItem.description}</p>
              </div>
              {viewDetailItem.details && (
                <div>
                  <span className="text-[10px] font-mono font-bold text-gray-400 uppercase block mb-1">{viewDetailItem.type === "websites" ? "Target domain URL" : "Payload Directive Context / Boilerplate text"}</span>
                  {viewDetailItem.type === "websites" ? (
                    <a href={viewDetailItem.details} target="_blank" rel="noopener noreferrer" className="text-xs font-mono text-blue-600 hover:underline flex items-center gap-1 bg-blue-50/50 p-2.5 border border-blue-100/60 rounded-lg select-all">
                      <Globe className="w-3.5 h-3.5 text-blue-500" /><span>{viewDetailItem.details}</span>
                    </a>
                  ) : (
                    <pre className="text-xs font-mono p-3 bg-gray-50 border border-gray-150 rounded-lg text-gray-800 leading-relaxed font-normal whitespace-pre-wrap select-all">{viewDetailItem.details}</pre>
                  )}
                </div>
              )}
              <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-3 text-xs font-mono">
                <div><span className="text-gray-400 block uppercase text-[9px] font-bold">Category Tags:</span><span className="text-gray-800 font-semibold">{viewDetailItem.tags}</span></div>
                <div><span className="text-gray-400 block uppercase text-[9px] font-bold">Authorized by:</span><span className="text-gray-800 font-semibold">{viewDetailItem.createdBy}</span></div>
              </div>
            </div>
            <div className="flex justify-between items-center pt-4 border-t border-gray-100 mt-5">
              <button onClick={(e) => handleDeleteItem(viewDetailItem.id, e)} className="text-xs font-mono font-bold text-rose-500 hover:text-rose-700 uppercase tracking-tight cursor-pointer">Remove record</button>
              <button onClick={() => setViewDetailItem(null)} className="px-5 py-2 border border-gray-250 bg-white hover:border-black rounded-lg text-xs font-mono uppercase tracking-tight transition cursor-pointer shadow-sm text-gray-800">Close View</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isAddFileOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-md bg-white border border-gray-200/90 shadow-2xl p-6 rounded-xl relative select-none">
            <button onClick={() => setIsAddFileOpen(false)} className="absolute right-4 top-4 p-1.5 text-gray-400 hover:text-black cursor-pointer transition border border-gray-100 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
            <div className="pb-3 border-b border-gray-100 mb-4">
              <span className="text-[10px] font-mono text-gray-400 uppercase tracking-widest block">FILE DEPLOYER</span>
              <h3 className="font-display font-extrabold text-base text-gray-900 mt-0.5">Upload and Synced Target Files</h3>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-mono uppercase text-gray-500 mb-1">Target Destination Folder:</label>
                <select value={formFolderTarget} onChange={(e) => setFormFolderTarget(e.target.value)} className="w-full bg-[#fcfcfc] border border-gray-200 rounded-lg p-2.5 text-xs text-gray-700 focus:outline-none focus:ring-1 focus:ring-black">
            ...
            <div className="flex justify-end space-x-2.5 pt-3 border-t border-gray-100 mt-4">
              <button type="button" onClick={() => setIsAddFileOpen(false)} className="px-4 py-2 border border-gray-200 text-gray-500 hover:text-black rounded-lg text-xs font-mono uppercase tracking-tight bg-white hover:bg-gray-50 transition cursor-pointer">Cancel</button>
            </div>
          </div>
        </div>
      )}
