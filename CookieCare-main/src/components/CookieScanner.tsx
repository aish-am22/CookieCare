...
export default function CookieScanner({ authToken }: CookieScannerProps) {
  ...
  // Refs to track active SSE/poll so they can be cleaned up on re-scan or unmount
  ...
  const handleStartScan = async (e: React.FormEvent) => {
    ...
    // Cancel any previous scan listeners
    cleanupListeners();
    ...
    try {
      ...
      if (res.status === 202 && data.job_id) {
        ...
        es.onerror = () => {
          // SSE dropped — poll fallback will cover it
          es.close();
          eventSourceRef.current = null;
        };

        // Poll fallback every 4s in case SSE misses the event
        ...
        // Return before finally so scanning state isn't cleared prematurely
        return;
      } else {
        setResult(data);
        setScanning(false);
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/reports/share-email"), {
        ...
        body: JSON.stringify({
          ...
          subject: `Lexify Cookie Compliance Scan - ${result.scanSummary.url}`,
...
Report secured and validated by Lexify FIPS Sandbox services.`;
  };
  ...
  const downloadReportFile = async (format: "pdf" | "docx") => {
    ...
    element.download = `Lexify_Compliance_${result?.scanSummary.url.replace(/https?:\/\/|www\./gi, "").replace(/[\.\s\/]/gi, "_")}.${format}`;
