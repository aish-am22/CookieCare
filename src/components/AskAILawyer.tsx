...
export default function AskAILawyer({ authToken, documents = [] }: AskAILawyerProps) {
  ...
  const triggerExport = async (format: "Word" | "PDF") => {
    ...
    try {
      ...
      console.warn("Backend export failed, falling back to local blob.", err.message);
      if (format === "Word") {
